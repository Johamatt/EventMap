...
export const fetchLinkedEvents = async (
  ...
  if (category === undefined) {
    try {
      const res = await axios.get(
        `https://api.hel.fi/linkedevents/v1/event?start=${startDateString}&end=${endDateString}&page=${page}&page_size=${size}&sort=start_time`,
        {
          headers: {
  ...
  try {
    const res = await axios.get(
      `https://api.hel.fi/linkedevents/v1/event?start=${startDateString}&end=${endDateString}&page=${page}&page_size=${size}&keyword=${category}&sort=start_time`,
      {
        headers: {
          "Access-Control-Allow-Methods": "GET",
          "Content-Type": "application/json",
